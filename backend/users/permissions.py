...
class IsDoctorSelf(BasePermission):
    def has_object_permission(self, request, view, obj):
        return request.user.role == 'doctor' and request.user.pk == obj.pk
...
class CanViewDoctorFullProfile(BasePermission):
    def has_object_permission(self, request, view, obj):
        if request.user.role in ('admin', 'receptionist'):
            return True
        if request.user.role == 'doctor' and request.user.pk == obj.pk:
            return True
        return False
...
class CanViewStaffModule(BasePermission):
    message = "Only administrators can access the staff module."
