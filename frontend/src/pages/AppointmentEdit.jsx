/* src/pages/AppointmentEdit.jsx - Route-based appointment edit form. */
  ...
  const loadAppointment = useCallback(async () => {
    ...
    try {
      ...
      const [patientResponse, doctorsResponse] = await Promise.allSettled([
        getPatient(getAppointmentPatientId(appointmentResponse)),
        getDoctors(),
      ])

      setAppointment(appointmentResponse)
      setPatient(
        patientResponse.status === 'fulfilled' ? patientResponse.value : null,
      )
      setDoctors(
          ...
          <>
            ...
            <div className="mt-8">
              <AppointmentFields
                doctors={doctors}
                errors={errors}
                lockPaid={appointment?.payment_status === 'paid'}
                register={register}
                requireFuture={false}
                setValue={setValue}
                watch={watch}
